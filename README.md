...
Output jar:
- `target/ashs-enchanting-1.0.11.jar`

## Install

1. Stop server.
2. Put `target/ashs-enchanting-1.0.11.jar` in `plugins/`.
...
- `/ashtetest`
  ...
  - Sets prior-work penalty (`RepairCost: 45`) so vanilla anvil logic is already past the `Too Expensive!` threshold.
